...
from networks.ensemble_resnet import ensemble_3_resnet18_fc, ensemble_5_resnet18_fc
...
parser.add_argument('--dataset', type=str, default='CIFAR100', help='datasets')#Tiny_Image,FashionMNIST
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        progress.progress_bar(
            batch_idx,
            len(trainloader),
            'Total_Loss: %.3f | Acc: %.3f%% (%d/%d)'%(train_loss /(batch_idx +1),
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        progress.progress_bar(
            batch_idx,
            len(testloader),
            'Total_Loss: %.3f | Acc: %.3f%% (%d/%d)' %(Test_loss /(batch_idx + 1),
                                                                     100. *float(correct)/total,
                                                                     correct,
                                                                     total))

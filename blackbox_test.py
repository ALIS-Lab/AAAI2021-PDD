...
from networks.ensemble_resnet import avg_ensemble_3_resnet18, avg_ensemble_3_resnet18_fc, avg_ensemble_3_resnet34, avg_ensemble_3_resnet34_fc
from networks.ensemble_vgg import avg_ensemble_3_vgg16_bn
...
parser.add_argument('--method', type=str, default='deg', help='methods:[baseline adp deg pdd pdd_deg]')
parser.add_argument('--black_attack_type', type=str, default='oblivious', help='black_attack_type:[oblivious adaptive]')
parser.add_argument('--surrogate_model', type=str, default='bs_vgg16', help='surrogate_model:[bs_vgg16 bs_resnet34 adp_resnet34 pdd_deg_resnet34]')
...
if opt.black_attack_type == 'oblivious':
    if opt.surrogate_model == 'bs_vgg16':
        black_model = avg_ensemble_3_vgg16_bn(num_classes)
        black_model_str = '/bs_ensemble_3_vgg16_bn'
if opt.black_attack_type == 'adaptive':
    if opt.surrogate_model == 'bs_resnet34':
        black_model = avg_ensemble_3_resnet34(num_classes)
        black_model_str = '/bs_ensemble_3_resnet34'
    if opt.surrogate_model == 'adp_resnet34':
        black_model = avg_ensemble_3_resnet34(num_classes)
        black_model_str = '/adp_ensemble_3_resnet34'
    if opt.surrogate_model == 'pdd_deg_resnet34':
        black_model = avg_ensemble_3_resnet34_fc(num_classes)
        black_model_str = '/pdd_deg_ensemble_3_resnet34_fc'

black_model_path = 'models/' + opt.dataset + black_model_str + '/best_model.pth'
...
black_classifier = PyTorchClassifier(model=black_model, loss=criterion, optimizer=None, clip_values=(0., 1.),input_shape=(1,3,32,32), nb_classes=num_classes)

def test_robust(opt, black_model, classifier, black_classifier, attack_method, c, norm=None):
    if opt.attack == 'FGSM':
        adv_crafter = FastGradientMethod(black_classifier, norm=norm, eps=c, targeted=False, num_random_init=0, batch_size=opt.bs)
    if opt.attack == 'PGD':
        adv_crafter = ProjectedGradientDescent(black_classifier,norm=norm, eps=c, eps_step=c / 10., max_iter=10, targeted=False,num_random_init=1, batch_size=opt.bs)
    if opt.attack == 'BIM':
        adv_crafter = ProjectedGradientDescent(black_classifier, norm=norm, eps=c, eps_step=c / 10., max_iter=10, targeted=False,num_random_init=0, batch_size=bs)
    if opt.attack == 'JSMA':
        adv_crafter = SaliencyMapMethod(black_classifier, theta=0.1, gamma=c, batch_size=opt.bs)
    if opt.attack == 'CW':
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        if opt.attack == 'CW':
            x_test_adv = adv_crafter(black_model, inputs, targets, to_numpy=True)
        else:
...
if __name__ == '__main__':
    ...
    print('Black Attack Type: {}, Surrogate Model: {}, Attack: {}'.format(opt.black_attack_type, opt.surrogate_model, opt.attack))
    ...
    for eps in epsilons:
        test_robust(opt, black_model, classifier, black_classifier, opt.attack, eps, norm=norm)